...
do_by_sims <- function(..., fun, args = NULL) {
  ...
  ## methods of capturing dots work (but do not work in all
  ## circumstances so still need lazy_dots just in case). Todo: fix
  ## this mess.

  try_dots <- try({
    dots <- lazy_dots(..., .follow_symbols = TRUE)
    env <- input_env()
    dots <- pluck(dots, "expr") %>% lapply(eval, envir = env)
    dots
  }, silent = TRUE)
  ...
  ## won't find the method. Defining a local copy resolves this. (???)
  ## See note in vapply help page?

  ## Note: tried to change the parent env of fun to the execution
  ## env of do_by_sims (`environment(fun) <- environment()`), but it
  ## didn't work.
  fnu <- function(...) fun(...)


  ## Small perf advantage for lapply in case of unary mappings
  res <- 
    if (length(dots) == 1)
  ...
  res %>% set_gsim_attributes("posterior")
}
...
`rbind_cols` <- operator(`_rbind_cols`)
`cbind_blocks` <- operator(`_cbind_blocks`)
`%*%.gs` <- operator(`%*%`)
`subset.gs` <- operator(`[`)
